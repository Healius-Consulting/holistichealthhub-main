...
export function planMultiRxPlacementRepair(input: {
  ...
  if (String(p1?.curaleafPrescriptionId) !== expected.overwrittenPrescriptionId) reasons.push('Prescription 1 is not in the investigated overwritten state');
  if (String(p1Sub.id) !== expected.overwrittenPurchaseOrderId
    || String(p1Sub.purchaseOrderId) !== expected.overwrittenPurchaseOrderId) reasons.push('unexpected current Prescription 1 purchase order');
  ...
  if (!p1Link || String(p1Link.placementState) !== 'PENDING_PLACEMENT' || p1Link.supplierPurchaseOrderId != null) reasons.push('unexpected Prescription 1 SQL link');
  ...
  if (!p1Serial || String(p1Serial.serialNumber) !== expected.serialNumber
    || String(p1Serial.curaleafPrescriptionId) !== expected.overwrittenPrescriptionId) reasons.push('unexpected Prescription 1 serial claim');
  ...
  const repairedSub = {
    ...
    items: asArray(p1Sub.items).map(item => ({ ...item, purchaseOrderId: expected.purchaseOrderId })),
  };
  return {
