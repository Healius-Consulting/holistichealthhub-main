...
test('only the known dummy pharmacy IDs sit in the training directory', () => {
  ...
  assert.equal(isTrainingDirectoryPharmacy({ id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', testAccount: true, tradingName: 'Sandbox' }), false);
  assert.equal(isTrainingDirectoryPharmacy({ id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', workspaceClassification: 'training', tradingName: 'Demo' }), false);
});
